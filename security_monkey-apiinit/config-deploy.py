...
def get_db_url():
    ...
    s3_object = client.get_object(Bucket=database_s3_bucket, Key=database_s3_key)
    db_config = json.loads(s3_object['Body'].read().decode('utf-8'))[database_name]
    return 'postgresql://{user}:{password}@{host}:{port}/{database}'.format(user=db_config['user'],
